...
export default function ResolutionCalculatorModal({ open, onClose }) {
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 pointer-events-none">
        <div className="bg-white rounded-xl shadow-xl w-full max-w-lg pointer-events-auto max-h-[90vh] overflow-y-auto">

          {/* Header */}
          <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-gray-100">
            <h2 className="text-sm font-semibold text-gray-900">Resolution Calculator</h2>
            <button
              onClick={onClose}
          ...
          <div className="p-6">
            ...
            <div className="grid grid-cols-2 gap-6 mb-6">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mb-2.5">
                  <label className="text-xs font-medium text-gray-600 w-12 shrink-0">Width</label>
                  <input
                ...
                <div className="flex items-center gap-2 mb-2.5">
                  <label className="text-xs font-medium text-gray-600 w-12 shrink-0">Height</label>
                  <input
                    type="number" min="0" step="any"
                    placeholder={`0 ${unitSuffix}`}
                    value={height}
                ...
                <div className="flex items-center gap-2 mb-4">
                  <label className="text-xs font-medium text-gray-600 w-12 shrink-0">DPI</label>
                  <input
              ...
              <div className="flex flex-col justify-center">
                {result ? (
                  <div className="space-y-3">
                    ...
                    {quality && (
                      <div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg border text-xs font-semibold ${quality.color}`}>
                        {quality.label}
                  ...
                  <p className="text-xs text-gray-300">Results appear here</p>
                )}
              </div>
            </div>
            ...
            <div className="border-t border-gray-100 pt-4">
              <h3 className="text-xs font-semibold text-gray-500 mb-3">Common Print Sizes</h3>
              <div className="space-y-2">
                {PRESET_GROUPS.map(group => (
                  <div key={group.name} className="flex items-start gap-2">
                    <span className="text-xs font-semibold text-gray-400 w-14 shrink-0 pt-1">{group.name}</span>
                    <div className="flex flex-wrap gap-1">
                      {group.sizes.map(size => (
                        <button
                          key={size.label}
                          onClick={() => handlePreset(size)}
                          className="px-2 py-0.5 text-xs rounded border border-gray-200 text-gray-600 hover:border-blue-300 hover:text-blue-700 hover:bg-blue-50 transition-colors"
                        >
                          {size.label}
                        </button>
                      ))}
