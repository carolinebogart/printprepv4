...
export const PORTRAIT_RATIOS = {
  '2:3': {
    ...
    sizes: [
      ...
      { width: 10,   height: 15,   label: '10x15'  },
      { width: 12,   height: 18,   label: '12x18'  },
      { width: 16,   height: 24,   label: '16x24'  },
      { width: 20,   height: 30,   label: '20x30'  },
      { width: 24,   height: 36,   label: '24x36'  },
      { width: 28,   height: 42,   label: '28x42'  },
      { width: 32,   height: 48,   label: '32x48'  },
      { width: 36,   height: 54,   label: '36x54'  },
  ...
  '3:4': {
    ...
    sizes: [
      ...
      { width: 15,   height: 20,   label: '15x20'  },
      { width: 18,   height: 24,   label: '18x24'  },
      { width: 24,   height: 32,   label: '24x32'  },
  ...
  '4:5': {
    ...
    sizes: [
      ...
      { width: 12,   height: 15,   label: '12x15'  },
      { width: 16,   height: 20,   label: '16x20'  },
      { width: 20,   height: 25,   label: '20x25'  },
      { width: 24,   height: 30,   label: '24x30'  },
      { width: 32,   height: 40,   label: '32x40'  },
...
export const LANDSCAPE_RATIOS = {
  ...
  '4:3': {
    ...
    sizes: [
      ...
      { width: 20,   height: 15,   label: '20x15'  },
      { width: 24,   height: 18,   label: '24x18'  },
      { width: 32,   height: 24,   label: '32x24'  },
  ...
  '5:4': {
    ...
    sizes: [
      ...
      { width: 15,   height: 12,   label: '15x12'  },
      { width: 20,   height: 16,   label: '20x16'  },
      { width: 25,   height: 20,   label: '25x20'  },
      { width: 30,   height: 24,   label: '30x24'  },
      { width: 40,   height: 32,   label: '40x32'  },
...
// Compute display specs for an image: simplified ratio, physical size at 300 DPI in inches and cm
export function getImageSpecs(width, height) {
  const gcd = (a, b) => (b === 0 ? a : gcd(b, a % b));
  const g = gcd(width, height);
  const rw = width / g;
  const rh = height / g;
  const ratioStr = rw <= 20 && rh <= 20 ? `${rw}:${rh}` : `${(width / height).toFixed(2)}:1`;
  const wIn = (width / 300).toFixed(1);
  const hIn = (height / 300).toFixed(1);
  const wCm = (width / 300 * 2.54).toFixed(1);
  const hCm = (height / 300 * 2.54).toFixed(1);
  return { ratioStr, wIn, hIn, wCm, hCm };
}
